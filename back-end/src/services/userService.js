...
const { ThrowerError, jwt } = require('../utils');
...
const login = async (e, password) => {
  ...
  if (!result) {
    throw ThrowerError('notFound', 'User not exists');
  }

  if (result.password !== md5(password)) {
    throw ThrowerError('unauthorized', 'Incorrect email or password');
  }
...
const createUser = async ({ name, email, password }) => {
  ...
  if (response) {
    throw ThrowerError('conflict', 'This email address is already in use');
  }
